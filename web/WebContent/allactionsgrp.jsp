...
<%@ page import="tasksentities.actiongrp" %>
<%@ page import="tasksentities.action" %>
...
<body class="gradient-custom-2">
    ...
    if(sess.getAttribute("access") == null || "0".equals(sess.getAttribute("access").toString())){
    ...
    if (idTaskStr == null || idTaskStr.trim().isEmpty()) {
        out.println("<p>Error: Missing 'idtask'. Please provide a task ID.</p>");
    } else {
        try {
            int id = Integer.parseInt(idTaskStr);
            EntityManagerFactory emf = Persistence.createEntityManagerFactory("taskproject");
            EntityManager em = emf.createEntityManager();
            em.getTransaction().begin();
            List<actiongrp> actions = em.createQuery("SELECT a FROM actiongrp a WHERE a.id_taskgrp.id = :id", actiongrp.class)
                                        .setParameter("id", id)
                                        .getResultList();
                ...
                <div class="card mask-custom">
                    <div class="card-body p-4 text-white">
                        <div class="text-center pt-3 pb-2">
                            ...
                            <h2 class="my-4">Actions List for Task ID: <%= id %></h2>
                        </div>
                        <table class="table text-white mb-0" style="background-color: aliceblue">
                            ...
                            <tbody>
                                <%
                                for (actiongrp action : actions) {
                                    int actionCreatorId = action.getId_user().getId();
                                %>
                                <tr class="fw-normal" style="color: rgb(43, 42, 42);">
                                    <td><%= action.getId() %></td>
                                    <td><%= action.getId_user().getNom() %></td>
                                    <td><%= action.getDescription() %></td>
                                    <td>
                                        <%
                                        if (sessionUserId == ownerid || sessionUserId == actionCreatorId) {
                                        %>
                                        <a href="modifyactiongrp.jsp?idtask=<%= action.getId()%>&description=<%= action.getDescription() %>&ownerid=<%=ownerid%>&taskid=<%=id%>" class="btn btn-primary btn-sm" title="Modify"><i class="fas fa-pencil-alt"></i></a>
                                        <a href="deleteactiongroupe?idt=<%= action.getId() %>&ownerid=<%=ownerid%>&taskid=<%=id%>" class="btn btn-danger btn-sm" title="Delete"><i class="fas fa-trash-alt"></i></a>
                                        <%
                                        } else {
                                        %>
                                        <span class="btn btn-secondary btn-sm disabled" title="Modify">Modify</span>
                                        <span class="btn btn-secondary btn-sm disabled" title="Delete">Delete</span>
                                        <%
                                        }
                                        %>
                                    </td>
                        ...
                        <%
                            ...
                            out.println("<p>Error: Invalid 'idtask' format. Please provide a numeric task ID.</p>");
                        } catch (Exception e) {
                            e.printStackTrace(); // Print stack traces to the page for debugging
                        }
                        }
                        %>
                    </div>
