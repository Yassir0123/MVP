                ...
                <div class="card mask-custom">
                    <div class="card-body p-4 text-white">
                        <div class="text-center pt-3 pb-2">
                            ...
                            <h2 class="my-4">Ajouter une action</h2>
                        </div>
                        <%
                        try {
                            int idtask = Integer.parseInt(request.getParameter("idtask"));
                            HttpSession sess = request.getSession();
                            int iduser = (int) sess.getAttribute("userid");
                        ...
                        %>
                        <form method="post" action="addactiongroupe">
                            <input type="hidden" name="id" value="<%= idtask %>"/>
                            <input type="hidden" name="iduser" value="<%= iduser %>"/>
                            <input type="hidden" name="ownerid" value="<%= ownerid %>"/>
                            <input type="hidden" name="passphrase" value="<%= passphrase %>"/>
                            <input type="hidden" name="idgrp" value="<%= grpid %>"/>
                            <div class="form-group">
                                <label for="description">Description</label>
                                <input type="text" class="form-control" id="description" name="description" required>
                            </div>
                            <button type="submit" class="btn btn-success">Ajouter action</button>
